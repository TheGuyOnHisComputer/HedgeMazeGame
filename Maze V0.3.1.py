...
def levelStartScreen():
    levelNameSurf1 = titleFont.render('Level Start', True, RED)
    levelNameRect1 = levelNameSurf1.get_rect()
    levelNameRect1.center = (WINDOWWIDTH / 2, WINDOWHEIGHT * 0.25)
    levelDescr = instFont.render('Use the arrow keys to navigate your way to the golden square(watch out for the hedges)!', True, BLUE)
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if (event.key == K_LEFT or event.key == K_a):
                    direction = LEFT
##                    print("Left")
                    if (x,y) in goalCoords and time.clock()<timer:
                        print(WINMESSAGE + "You took " + str(time.clock())+" Seconds.")
                        time.sleep(3)
                        terminate()
                    elif (x,y) in goalCoords:
                        print("You took too long; have another go(but do better)!")
                        loadlevel1()
